...
def compute_test_series(test):
    ...
    failures = (groups.any().astype({test.FAILED: float})
                    .groupby([meas]).mean()[test.FAILED])
...
def compute_train_series(train, window=10):
    ...
    failures = (groups.any().astype({train.FAILED: float})[train.FAILED]
...
if __name__ == '__main__':
    ...
    expname = full if expnum is None else f'full_test_{expnum}'
    ...
    train_reward, train_failures = compute_train_series(train, window=10)
