...
class TestGP(unittest.TestCase):
    def _matern_52(self, x, outputscale, lengthscale):
        return outputscale ** 2 * (
                1 + np.sqrt(5) * x / lengthscale + 5 * (x ** 2) / (
                    3 * lengthscale ** 2)
        ) * np.exp(
    ...
    def test_hyper_optimization_1(self):
        ...
        y = self._matern_52(x, lengthscale=lengthscale,
        ...
        gp = MaternGP(x_train, y_train, nu=5 / 2, noise_constraint=(0, 1e-5),
    ...
    def test_hyperparameters_2(self):
        ...
        passed = np.all(np.abs(y_test - y_pred) < tol)
        passed &= np.abs(
            gp.covar_module.base_kernel.period_length.item() - period_length) < tol

        if passed:
            self.assertTrue(True)
        else:
            print(
                f'Period length: predicted= {gp.covar_module.base_kernel.period_length.item()} | true= {period_length}')
            print(f'Max(diff): {np.abs(y_test - y_pred).max()}')
            x_train = x_train.reshape(-1)
            x_test = x_test.reshape(-1)
            f, ax = plt.subplots(1, 1, figsize=(8, 8))
...
class TestValueStructureGP(unittest.TestCase):
    noise = 0.01
    CONSTANT = 'constant'
    LINEAR = 'linear'
    mode = 'constant'
    ...
    def _reward(self, action):
        if self.mode == self.CONSTANT:
            return np.ones_like(action, dtype=float).reshape(-1)
        else:
            return np.atleast_1d(action).reshape(-1)

    def get_examples(self, n):
        actions = np.random.uniform(0, 1, n).reshape((-1, 1))
        rewards = self._reward(actions).reshape(-1) + \
                  self.noise ** 2 * np.random.randn(actions.shape[1])
        return actions, rewards

    def get_truth(self, n, gamma):
        actions, rewards = self.get_examples(n)
        if self.mode == self.CONSTANT:
            return (
                actions,
                np.array([1/(1 - gamma)]*n)
            )
        else:
            return (
                actions,
                actions.reshape(-1) + 0.5 * gamma / (1 - gamma)
            )
    ...
    def test_creation(self):
        gp = self.get_gp()
        self.assertTrue(isinstance(gp.covar_module, ValueStructureKernel))
