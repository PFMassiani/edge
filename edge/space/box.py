...
class Segment(DiscreteSpace):
    ...
    def _get_closest_index(self,x):
        return np.around(self.n_points * (x - self.low)/(self.high - self.low))
    ...
    def indexof(self,x):
        ...
        if abs(self._get_value_of_index(index) - x) > self.tolerance:
            raise error.OutOfSpace(f'Value {x} is between the bounds of the Segment, but is not on the discretization grid')
    ...
    def __init__(self,low,high,shape):
        if isinstance(low,Number) and isinstance(high,Number):
            ...
            if not (low.shape == high.shape) and (low.shape == shape):
                raise ValueError(f'Shape mismatch. Low {low.shape} High {high.shape} Shape {shape}')
            self.dim = len(shape)
